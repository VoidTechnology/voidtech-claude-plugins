...
- 摘要：记录 2026-06-26 可移植性审计当时的发现与整改，不代表当前插件数量、版本或门禁状态。
...
- `voidtech-core` 恰好发布约定的 22 个技能命令；目录名、frontmatter 展示名和内部引用已经同步。
- 内部编排技能 `plan-review-core` 不在用户命令菜单中展示，但仍可由模型调用。
- 22 个核心技能已逐项完成自洽性审计；本地引用、跨技能调用、随附脚本定位和许可证由自动检查约束。
- 22 个核心技能及其参考文件已完成中文可读性审查；已淘汰的生硬译法由自动检查约束。
