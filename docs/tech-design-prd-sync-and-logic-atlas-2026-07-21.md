...
- **日期**：2026-07-21（2026-07-24 修订）
- **状态**：Final（2026-07-22 终审批准：ADR ACCEPTED/FROZEN、技术设计 APPROVED、实现 AUTHORIZED、剩余架构阻塞 0）
- **实施状态**：§12 五个阶段门全部通过（2026-07-23）——事务地基、迁移、只读同步、受控合入、Logic Atlas 均已落盘，§13 完整交付机械验收全部满足。门 2–4 由 voidtech-loop 驱动实现并经建议模式审查后人工 accept；门 5 worker 中断后由主会话打捞收尾。skill 层接线已交付（2026-07-23，voidtech-core 0.12.0）：`prd-sync` 公共技能与 CLI（13 个子命令，退出码 0/1/2/3/4 契约）、`prd-maintain` 工况 5 与能力分层收尾、`check-prd-tree.py` §9 改造（只读栅栏退出码 3、reconciliation 排除、`--operation-id` overlay、`markdown_validator` 拆分）、渲染器浏览器 CI（ADR-0005 §8：零依赖 CDP harness、七键验证证明、renderer-validation workflow）。Behavioral Atlas 于 2026-07-24 升级至 generator 1.3.0 + viewer 5.0.0：在单场景业务主线下按选中步骤展开页面交互，精确挂载状态、异常与依赖，支持限定跨模块页面引用并 fail closed；Example「入会审批与缴费」已发布 3 步、12 条页面交互、3 条状态影响、13 条页面异常且目标模块无交互关联 gap，真实浏览器完成步骤切换、键盘、来源抽屉、窄屏裁切、零外链与零 console error 验收。ADR-0004 第三阶段（overriding 完整流程、复活候选、多可更新源）为后续工作
...
**问题。** 现有场景图能回答角色、业务动作、状态变化、异常与依赖，但一条 `flowStep` 往往把「进入页面 → 点击控件 → 编辑表单 → 提交 → 等待 → 成功或恢复」压缩成一句业务描述，无法直接指导页面设计、前端实现与交互验收。

**决策。** Behavioral Atlas 采用两层行为模型，而不是把按钮、状态与异常继续堆进主流程：

1. **业务场景层**保留 `flowStep` 主干，只回答谁在何处完成什么任务，以及成功后的业务结果。
2. **页面交互层**在选中某个 `flowStep` 后展开该步骤的操作链；每个交互节点描述一个有产品意义的用户或系统动作，成功关系按 `下一交互` 串联，状态变化、页面异常与依赖挂到实际触发它们的交互。

页面、抽屉、弹窗与关键 UI 状态是交互发生的上下文；用户可见控件与事件是动作。不得为普通滚动、hover 或无业务影响的展开折叠建节点。只有页面/容器切换、写操作、异步提交、权限或状态 gating、不可逆动作，以及验收标准要求判断的交互进入模型。

**机器契约。** 模块 PRD 新增 `### 5.0.3 页面交互（机器可解析）`，固定表头与列序如下：

| 流程 | 步骤ID | 交互ID | 页面 | 容器/状态 | 控件 | 事件 | 可用条件 | 即时反馈 | 系统动作 | 成功结果 | 失败与恢复 | 下一交互 | 需求编号 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| {流程名称} | {S1} | {I1} | {页面名} | {列表/详情/编辑态/弹窗等} | {用户可见控件文案/无} | {进入/点击/输入/选择/提交/系统触发} | {权限、业务状态和字段条件} | {打开/禁用/Loading 等} | {读取、校验、写入或触发事件} | {成功后的页面与反馈} | {失败提示、内容保留和恢复操作} | {I2/结束} | {REQ-ID} |

约束：

- `流程`、`步骤ID` 必须引用 §5.0.1。`页面` 的本模块页面写 §5.0 页面名；跨模块页面使用 `<module-scope>::<页面名>`（例如 `03-portal/04-member-center::会籍资料`），目标模块和页面节点都必须存在。模块存在但页面契约尚未结构化时进入 `gaps`，不得退化成同名猜测。
- `交互ID` 在同一流程步骤内唯一；`下一交互` 只能引用同一步骤内 ID 或「结束」。
- 每个有交互表的业务步骤恰有一个入口；成功链不得断头或循环，必须可达「结束」。
- 控件写用户可见文案；PRD 未声明的页面形态、按钮、确认层级或反馈不得猜测，进入 `gaps`。
- 事件只允许「进入 / 点击 / 输入 / 选择 / 提交 / 系统触发」；无控件的进入或系统动作写「无」。
- 状态影响表增加 `交互ID`，把业务状态变化绑定到产生它的提交动作；试点边缘状态表增加 `交互ID`，把异常绑定到可恢复的具体动作。旧流程未深化到交互层时保留步骤级或流程级缺口，不自动映射。

**读模型。** 沿用 logic-model schema v1 的开放 `detail` 与现有 `flow` / `navigates` / `traces` 类型，新增 `interactionStep` category、交互成功关系及 interaction 到 flowStep/page/stateImpact/pageState 的追溯。无需 schema 大版本升级；generator 版本递增。编译器必须验证流程、步骤、页面、交互唯一性、下一交互、唯一入口与可终止性，所有坏引用进入 `gaps`。

**渲染。** 场景主干保持单场景、单一成功路径。点击 S1/S2/S3 后，图下只展开当前步骤的交互轨迹，默认选中首步；交互卡展示页面/容器、控件与事件、可用条件、即时反馈、系统动作和成功结果，异常默认折叠，状态变化挂到触发交互。现有来源抽屉保留为二级入口。主流程箭头只连接业务步骤，不得穿过状态或异常区域。

**试点与边界。** 第一轮只深化 Example「入会审批与缴费」三步。S1 后台审批与 S3 后台确认到账完整建模；工作树核对确认 S2 属于门户 `03-portal/04-member-center` 的「会籍资料」页面（PTL-056~060），并为该页面补入最小机器契约后使用限定引用，不把机构后台「会员订单」伪装成申请人页面，也不扩写该骨架模块的其他页面。其他六个场景不补造交互数据。此次不产出像素级页面设计、不规定 API 字段与数据库实现，也不记录普通微交互。

**产品验收。** 不打开完整 PRD，评审者应能从选中步骤回答：用户在哪个页面、操作哪个控件、何时可用、立即反馈是什么、系统执行什么、成功后界面和业务状态如何变化、失败后如何恢复、下一交互在哪里。真实浏览器验证覆盖步骤切换、单轨迹展开、状态/异常/依赖精确挂载、来源入口、键盘可达、无横向裁切、无 console 错误与零外链。
...
| 2026-07-24 | 页面交互轨迹方案冻结：Behavioral Atlas 从单层业务行为图扩展为「业务场景 + 步骤内页面交互」两层模型；新增 §5.0.3 固定表契约、interactionStep 读模型、状态/异常到交互的精确绑定、唯一入口与可终止性校验、内联交互轨迹渲染和 Example 三步试点边界 | 现有 flowStep 把多个页面操作压缩成一句业务动作，无法直接指导设计、前端和 QA；两层模型保留业务故事可读性，同时补足按钮、即时反馈、成功与恢复路径 |
| 2026-07-24 | 页面交互轨迹完成：generator 1.3.0 编译 `interactionStep`、成功链与 step/page/stateImpact/pageState 追溯，统一解析本地及限定跨模块页面引用并校验唯一入口、断头、循环、可终止性；viewer 5.0.0 改为主流程摘要 + 选中步骤交互卡网格，状态/异常精确挂载，箭头对齐，来源抽屉与依赖泳道保留；renderer harness 5.0.0 增加步骤切换、跨步骤附件隔离及页面级裁切防护。Example 试点发布 3 步、12 交互、3 状态影响、13 页面异常，S2 精确引用门户会籍资料页面 | 将冻结方案落为可供产品、设计、前端与 QA 共用的可执行交互故事，同时避免申请人缴费页误指后台页面 |
| 2026-07-24 | 语义图形化完成：viewer 6.0.0 按问题类型拆分视觉语法——场景主流程使用角色泳道 Workflow 与正交条件箭头，页面交互卡使用字段级 SVG 图标和图例，生命周期视图标识起点/过程/终点，系统关系图显示架构节点类型；Markdown 仍以固定表作为唯一权威输入，图形仅为确定性读模型。renderer harness 6.0.0 增加泳道、连线、语义图标、生命周期图例和架构图标断言 | 解决「所有信息都像表格/卡片」导致主线、角色切换、状态语义和系统层次需要靠逐字阅读才能理解的问题，同时避免引入外部图形运行时或第二权威源 |
| 2026-07-24 | 质量审计入口完成：generator 1.4.0 新增需求摘要、页面数据显式读写关系、验收级六类结构门禁、状态动作/结果拆分与显式终态；viewer 7.0.0 默认进入覆盖率和模块健康驾驶舱，提供需求反向追溯，折叠模板型缺口，恢复来源精确锚点并清除 Markdown/本地路径泄漏；renderer harness 7.0.0 覆盖审计首页、下钻、视图切换与无回归断言。Example 5 个验收级模块完成真实数据试点并发布 | 原 Atlas 有图但不先回答「PRD 是否完整、哪块最危险、需求影响了什么」；本批次将其收口为可判定、可下钻、可追溯的发布前审计工具 |
| 2026-07-24 | Lifecycle 状态机接线完成：generator 1.5.0 以 Mermaid 逐边来源替代按状态表笛卡尔积，支持共享图按业务对象裁切；确定性 IR 以 SCC/最长路径布置状态，Archify bridge 在 8 轮内按机器诊断受限修复并按单图降级；viewer/renderer harness 8.0.0 内联清理后的 SVG、保留来源追溯并覆盖 Node 缺失降级。Example 全局 13 个状态机 deliver 全部通过，用户与会员模块 6 张图浏览器验收状态标签唯一且无降级 | 原状态机视图复制状态节点，且编译器把按状态动作整团复制到每条出边；新链路同时解决可读性、逐边语义正确性与呈现失败不阻塞内容门 |
