...
- `流程`、`步骤ID` 必须引用 §5.0.1；`页面` 必须引用 §5.0 页面契约。
...
**试点与边界。** 第一轮只深化 Example「入会审批与缴费」三步。S1 后台审批与 S3 后台确认到账完整建模；S2 申请人支付先在全工作树核对页面归属，找不到权威页面时显式保留跨端 gap，不伪造门户页面。其他六个场景不补造交互数据。此次不产出像素级页面设计、不规定 API 字段与数据库实现，也不记录普通微交互。
