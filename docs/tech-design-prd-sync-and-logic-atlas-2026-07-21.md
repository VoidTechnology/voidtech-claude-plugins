...
**试点与边界。** 第一轮只深化 Example「入会审批与缴费」三步。S1 后台审批与 S3 后台确认到账完整建模；工作树核对确认 S2 属于门户 `03-portal/04-member-center` 的「会籍资料」页面（PTL-056~060），但该骨架模块尚无机器可解析页面契约，因此试点使用限定引用并如实产生「跨模块页面未结构化」gap，不把机构后台「会员订单」伪装成申请人页面。其他六个场景不补造交互数据。此次不产出像素级页面设计、不规定 API 字段与数据库实现，也不记录普通微交互。
