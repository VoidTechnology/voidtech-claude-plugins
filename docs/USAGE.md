...
- **领域词汇共享**：多个技能在动手前会读取 `CONTEXT.md` 与 ADR，使用项目领域术语表的措辞。先用 `domain-modeling` 建好模型，后续技能的产出质量会显著提升。
...
| `plan-review-docs` | 同上，并同步整理 ADR 与领域术语表 | 审查结论 + 文档更新 |
...
- **领域模型是前置投资**：在新项目或新领域先跑一次 `domain-modeling`，后续所有技能都会复用其术语与 ADR。
