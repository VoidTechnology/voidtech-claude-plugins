...
现有工作流不是一堆零散命令，而是按交付责任拆开的**自包含工程工作流**：
...
```text
/voidtech-core:<skill>
/voidtech-product:<skill>
/voidtech-design:<skill>
/voidtech-engineering:<skill>
```
...
| 可见性 | 含义 | 技能 |
|---|---|---|
| **模型可援引** | 命中场景时 Claude 可以主动使用，也可以手动调用；涉及文件或仓库状态变更时，按技能自己的流程确认 | `voidtech-engineering:codebase-design`、`debug`、`feature-context`、`fix-conflicts`、`git-safety`、`setup-git-checks`、`tdd`；`voidtech-core:text-naturalizer` |
| **仅用户显式触发** | 需要明确入口，可能带来较大副作用、网络/成本开销，或代表一次完整工作流授权；必须你手动 `/` 调用 | Core：`handoff`、`learn`、`plan-review`、`plan-review-docs`、`research`、`write-skills`；Product：全部；Design：全部；Engineering：`architecture-review`、`implement`、`logic-spike`、`prepare-issue`、`ship`、`to-issues`、`to-prd` |
| **仅内部编排** | 不出现在命令菜单，由其他技能调用 | `voidtech-core:plan-review-core` |
...
| 命令 | 用途 | 主要产出 |
|---|---|---|
| `voidtech-core:research` | 对不熟悉的问题做多信源开放网络调研，汇总证据、分歧、可信度和建议 | 调研结论、证据表、风险、下一步验证 |
| `voidtech-engineering:feature-context` | 把已经确认的仓库业务术语和概念边界写入上下文；仅为难逆转且存在真实取舍的决策提议 ADR | 更新后的 `CONTEXT.md`，必要时为 ADR |
| `voidtech-engineering:codebase-design` | 设计接口简单、内部完整的深模块，确定可替换 seam | 模块/接口设计、seam 方案 |
| `voidtech-engineering:to-prd` | 把当前对话综合成工程交付 PRD（不重新访谈） | 发布到跟踪器的 PRD，或 Markdown 草稿 |
...
| `voidtech-core:plan-review` | 逐项检查方案，找遗漏、冲突和未验证假设 | 审查结论 |
| `voidtech-core:plan-review-docs` | 同上，并同步整理 ADR 与业务词汇表 | 审查结论 + 文档更新 |

### 实现与验证

| 命令 | 用途 | 主要产出 |
|---|---|---|
| `voidtech-engineering:implement` | 按已有 PRD 或一组 issue 实现、测试、自审、交付检查 | 已验证的工作树 + 验收证据 + 变更摘要 |
| `voidtech-engineering:tdd` | 红→绿→重构小步循环，通过公开行为验证 | 测试 + 实现 |
| `voidtech-design:ui-prototype` | 在同一路由比较多种一次性 UI 结构方案 | 可运行 UI 原型 + 设计选择；验证后清理变体和切换器 |
| `voidtech-engineering:logic-spike` | 用一次性终端程序推演状态、逻辑或数据结构 | 可运行 TUI + 模型结论；验证后清理外壳 |
| `voidtech-engineering:debug` | 建稳定复现→二分定位→根因→补回归测试 | 修复 + 防回归测试 |
| `voidtech-engineering:architecture-review` | 扫描整个代码库，寻找整合浅模块、简化接口的候选项；由用户选中后再深入审查 | 临时 HTML 候选报告 + 选中方案审查 |
| `voidtech-engineering:ship` | 审查 diff、运行验证、提交、推送并创建 PR/MR | 远端 PR/MR + 验证摘要 |

### Git 与安全

| 命令 | 用途 | 主要产出 |
|---|---|---|
| `voidtech-engineering:git-safety` | 配置钩子，拦截 push/reset --hard/clean/branch -D 等危险命令 | Claude Code hook 配置 |
| `voidtech-engineering:setup-git-checks` | 配置 Husky 预提交：lint-staged(Prettier)+类型检查+测试 | 预提交钩子 |
| `voidtech-engineering:fix-conflicts` | 解决进行中的 merge/rebase 冲突，保留双方意图并验证 | 已解决冲突的工作树 |

### 协作与文档

| 命令 | 用途 | 主要产出 |
|---|---|---|
| `voidtech-core:handoff` | 把当前任务整理成交接文档供另一 agent 接力 | 临时目录中的 handoff 文件 |
| `voidtech-core:learn` | 跨多个会话持续教授一个主题，根据学习目标整理可信资料、短课程、练习和进度 | `MISSION.md`、课程、资料与学习记录 |
| `voidtech-core:text-naturalizer` | 润色中/英/混排文本，去除 AI 写作痕迹 | 改写后的文本 |
| `voidtech-core:write-skills` | 编写/改进技能，保持触发准确、流程清楚、可验证 | 新建或优化的 SKILL.md |
...
| 当前情况 | 应使用 | 不是 |
|---|---|---|
| 当前对话已经把需求讨论清楚，要整理成一份单体 PRD | `voidtech-engineering:to-prd` | 重新访谈需求，或生成模块化工作树 |
| 手上是原始需求、Excel、访谈纪要、需求清单或旧 PRD，要建立可追溯的模块化主本 | `voidtech-product:prd-from-requirements` | `to-prd` 的后续步骤 |
| 已经有 `prd-from-requirements` 生成的工作树，要深化、合入变更或落实评审意见 | `voidtech-product:prd-maintain` | 重新运行生成流程 |
| 需求源（如 Excel）出了新版本，要与既有工作树安全归并，且不覆盖已有裁决 | `voidtech-product:prd-sync` | 重新运行生成流程，或让 `prd-maintain` 直接照描述改主本 |
| 已知具体模块，需要设计或改进它的 interface、seam 与可测试性 | `voidtech-engineering:codebase-design` | 全仓架构体检 |
| 尚不知道最值得改哪里，要扫描全仓并比较多个模块深化候选 | `voidtech-engineering:architecture-review` | 直接实施重构；它先生成报告，等用户选择候选项 |
...
| 要进行跨会话的系统学习，并持续维护课程、可信资料、练习和进展 | `voidtech-core:learn` | 回答一次技术问题或临时查资料 |

### 日常实现：走最短有效路径

```text
范围明确、没有 PRD/issue
   └─▶ /voidtech-engineering:tdd           确认接口与关键行为，逐个红→绿→重构

已有 PRD 或一组 issue
   └─▶ /voidtech-engineering:implement     实现、测试、逐条核对验收标准、自审
       └─▶ 技能内部尽可能使用 tdd

工作树已验证，且明确要提交、推送并创建 PR/MR
   └─▶ /voidtech-engineering:ship          审查 diff、运行质量门、提交、推送、建 PR/MR
```
...
```text
当前对话已讨论清楚 ──▶ /voidtech-engineering:to-prd
原始需求/Excel/访谈纪要 ──▶ /voidtech-product:prd-from-requirements
既有模块化 PRD 工作树 ──▶ /voidtech-product:prd-maintain
需求源出了新版本要导入 ──▶ /voidtech-product:prd-sync（裁决后交 prd-maintain 合入）
```

`logic-atlas.html` 不是建树就有的产物：`prd-from-requirements` 只生成状态看板，Atlas 要在工作树迁移后由 `prd-sync` 显式置位能力（`atlas <工作树> --enable <markdown|html|polish>`），再由 `prd-maintain` 或 `prd-sync` 运行 `atlas --publish` 生成。置位时机见 `prd-from-requirements` 的「Logic Atlas 置位时机」——骨架级阶段开启只会得到一张几乎全是「未声明」的图。

生成后的 `logic-atlas.html` 默认从「探索」进入：先按任务查看场景、页面、数据与字段、访问规则、需求和来源；「质量与来源」只展示机械覆盖、内容深度、缺口和快照新鲜度，不代表 PRD 内容已经正确。页面详情首屏回答入口、主体、动作与去向，其余关系按需展开。没有来源的旅程、页面跳转、字段或权限关系会显示为未声明，不会按标题相似度补齐。

得到计划、规格或 PRD 后，只有确实需要多人认领或拆成多个独立交付单元时才继续：

```text
计划/规格/PRD
   └─▶ /voidtech-engineering:to-issues     与用户确认垂直切片后发布 issue
       └─▶ /voidtech-engineering:implement 按 issue 实现与验证
           └─▶ （可选）独立审查
               └─▶ /voidtech-engineering:ship 明确授权后发布远端变更
```
...
```text
/voidtech-engineering:debug 建稳定复现 → 最小化 → 排序并验证假设 → 修复 → 回归测试
...
```text
/voidtech-engineering:prepare-issue 按 bug/enhancement 分类、按 5 种状态流转、验证主张、写 agent 实现说明
   └─▶ ready-for-agent 的 issue 可交给 /voidtech-engineering:implement
```
...
```text
/voidtech-engineering:git-safety        配置 Claude Code 钩子，拦截破坏性 git 命令
/voidtech-engineering:setup-git-checks  为 JS/TS 仓库配置 Husky 预提交门禁
/voidtech-engineering:fix-conflicts     仅在 merge/rebase 已发生冲突时调用
```
...
| 我想… | 用 |
|---|---|
| 对陌生问题做开放网络调研并拿到建议 | `voidtech-core:research` |
| 把当前对话中已经讨论清楚的需求整理成单体 PRD，不重新访谈 | `voidtech-engineering:to-prd` |
...
| 审查、提交、推送并创建 PR/MR | `voidtech-engineering:ship` |
| 没有正式 PRD/issue，但要测试先行实现一个范围明确的行为 | `voidtech-engineering:tdd` |
| 某个东西坏了/变慢了/偶发失败 | `voidtech-engineering:debug` |
| 整理别人提的 issue 或外部 PR | `voidtech-engineering:prepare-issue` |
| 动手前逐项审查方案；还要同步已确认的词汇/ADR 时选择 docs 版本 | `voidtech-core:plan-review` / `voidtech-core:plan-review-docs` |
| 为一个已知模块设计更深的 interface 与 seam | `voidtech-engineering:codebase-design` |
| 扫描全仓寻找架构改进候选，再挑一个深入审查 | `voidtech-engineering:architecture-review` |
| 把会话中已经确认的新业务术语、概念边界或重要决策写回项目 | `voidtech-engineering:feature-context` |
| 防止误删/误推/误重置 | `voidtech-engineering:git-safety` |
| 给 JS/TS 仓库加 Husky 提交前格式、类型和测试门禁 | `voidtech-engineering:setup-git-checks` |
| 卡在合并冲突里 | `voidtech-engineering:fix-conflicts` |
| 换会话/交给别人继续 | `voidtech-core:handoff` |
| 按明确目标跨多个会话系统学习一个主题 | `voidtech-core:learn` |
| 让 PR 描述/文档读起来不像 AI 写的 | `voidtech-core:text-naturalizer` |
| 给团队加一个新技能 | `voidtech-core:write-skills` |
...
四个工作流插件本身不绑定外部服务，但下列配合能放大效果：

- **GitHub / GitLab / issue 跟踪器**：GitHub 先 `gh auth login`，GitLab 先 `glab auth login`。`to-prd`、`to-issues`、`prepare-issue` 会按 [Issue 跟踪器适配契约](../plugins/voidtech-engineering/skills/_shared/ISSUE-TRACKER.md) 探测平台与标签；跟踪器不可用时退化为本地 Markdown 草稿。
- **可选 MCP 插件**（默认禁用，按需 `claude plugin enable`）：
  - `voidtech-mcp-common`：Context7（查库文档）、Chrome DevTools（无头浏览器验证，对 `voidtech-engineering:debug` / `voidtech-design:ui-prototype` 的 UI 验证很有用）。
...
| 实现前端界面或比较原型 UI | `frontend-design`、`figma` | 生产级 UI 直接使用 `frontend-design`。只有要比较多个结构方案时，才先用 `voidtech-design:ui-prototype`；选定方向后清理临时代码，并补齐生产级实现。需要设计稿上下文时接入 `figma` |
...
- **原型代码不是生产代码**：`voidtech-design:ui-prototype` 与 `voidtech-engineering:logic-spike` 各自回答一个明确问题；验证后记录答案并删除切换器、一次性路由或终端外壳。
- **功能上下文按需更新**：相关技能可以直接读取已有 `CONTEXT.md` 与 ADR；只有新增、修改或澄清仓库上下文时才调用 `voidtech-engineering:feature-context`，且 `CONTEXT.md` 只记录业务词汇，不承载产品规格或实现细节。
- **技能归属是公共接口**：扩展或重命名技能时走 `voidtech-core:write-skills`，同步更新 [ADR-0007](decisions/0007-voidtech-suite-plugin-architecture.md) 与可移植性检查。
...
`voidtech-engineering:implement` 适合人在会话中跟进、按 PRD 或 issue 完成一次实现；`voidtech-loop:goal` 适合验收命令已经明确、希望控制器在后台多轮推进的任务。二者不会自动互相启动。
