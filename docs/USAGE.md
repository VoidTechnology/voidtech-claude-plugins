...
`prd-from-requirements` 生成的 `logic-atlas.html` 默认从「探索」进入：先按任务查看场景、页面、数据与字段、访问规则、需求和来源；「质量与来源」只展示机械覆盖、内容深度、缺口和快照新鲜度，不代表 PRD 内容已经正确。页面详情首屏回答入口、主体、动作与去向，其余关系按需展开。没有来源的旅程、页面跳转、字段或权限关系会显示为未声明，不会按标题相似度补齐。
