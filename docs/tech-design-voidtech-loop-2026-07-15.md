...
| 8.11 | 宿主模型 | detach 守护进程 + 启动握手 | spike 第二轮实测 + 一期 1.1 收尾 |
| 8.12 | 验收依赖 | 每个干净 worktree 执行 Goal Spec `setup` | 一期 1.1 语义定案 |
...
- `goal` 完成前台准备后以 detach 子进程派生控制器，并通过专用 IPC 等待控制器完成锁与状态身份接管；只有收到 ready 握手才返回 run ID。握手失败统一终态化并释放项目锁，不假报启动成功。
...
- **稳定语义承诺**：Goal Spec 可选字段 `setup` 是 shell 命令字符串数组并进入 `goal_hash`；每条命令按顺序在基线 worktree、循环 worktree 与每次验收的一次性 worktree 中各执行一遍。未声明 `setup` 时不自动推断安装命令。
- 每条 setup 命令超时 900 秒。命令产物必须由仓库 `.gitignore` 覆盖，否则循环 worktree 中的产物会被视为 worker 变更进入 checkpoint。
- `goal-spec baseline` 与 `loop goal` 共用一份 shell 确认门；存在 `setup` 或 `shell: true` eval 时，两个入口都必须先完整展示命令，并仅在显式传入 `--allow-shell` 后执行。
- 基线、循环和验收均从指定 commit 创建干净 worktree；setup 的执行次数、candidate 绑定与可观察结果属于契约，后续优化不得改变。
- **未来性能优化**：允许用预热安装、APFS clonefile 或等价机制预填依赖产物，但只能作为执行 setup 前的缓存；不得跳过命令、复用可观察状态或改变上述语义。
- **注**：`setup` 是本设计在 PRD schema 示例之外新增的唯一可选字段，由共享 schema 定义并进入 `goal_hash`。

## 变更记录

- 2026-07-15：初稿即定稿（Final）——十二项全部定案；`setup` 字段为相对 PRD 的唯一 schema 增项。
- 2026-07-16：一期 1.1 收尾——§8.11 增加 ready 握手与准备失败终态化；§8.12 将“每 run 预热安装 + clonefile”修订为“每个干净 worktree 执行 setup”的稳定语义契约，预热与克隆仅保留为未来性能优化；baseline 与正式启动共用 shell 确认门。
