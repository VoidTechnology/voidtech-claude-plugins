...
`Claude Code ≥ 2.1.154` · `voidtech-core v0.9.0` · 内部分发

VoidTech 团队的 Claude Code Marketplace。它把团队约定固化成可独立安装、可验证、许可证明确的插件，让每位成员开箱即用同一套工程方法论，而把高权限的 MCP 留作按需启用。

## 亮点

- **23 个自包含工程技能**，覆盖从调研到交付的完整生命周期（调研、规划、设计、实现、调试、Git、协作），不依赖插件目录之外的脚本或运行时。
...
```text
/voidtech-core:research      调研陌生问题并汇总证据与建议
...
- claude plugin install voidtech-core@voidtech
voidtech-core 提供 23 个自包含中文工程技能 + 中文协作约定（SessionStart hook）。
...
| 插件 | 版本 | 默认 | 内容 |
|---|---|---|---|
| [`voidtech-core`](plugins/voidtech-core) | 0.9.0 | ✅ 启用 | 中文约定 + 23 个自包含工程技能 |
...
| PR/MR 二次审查 | `pr-review-toolkit` 或 `code-review` | 放在 `/voidtech-core:ship` 前后做独立审查，二选一即可 |
| 前端视觉实现 | `frontend-design` | 和 `prototype` 搭配，补足高质量 UI 设计与实现 |
...
- **调研、规划与设计** — `research`、`feature-context`、`codebase-design`、`to-prd`、`to-design-brief`、`to-issues`、`prepare-issue`、`plan-review`、`plan-review-docs`
...
plugins/
  voidtech-core/                  中文约定 + 23 个自包含工程技能（含 SessionStart hook）
...
前置条件：Claude Code 2.1.154 或更高版本、`jq`、`rg`、`gh`、`glab`。
