...
`voidtech-core v0.11.2` · `voidtech-loop v0.1.0` · Claude Code ≥ 2.1.154（loop ≥ 2.1.210） · 内部分发
...
`voidtech-loop` 当前处于 F3 开发阶段，仅支持 macOS arm64；不满足试点环境时可以只安装 `voidtech-core`。
...
- 提醒我：voidtech-loop 当前处于 F3 开发阶段；不满足试点条件时跳过 loop，不影响 core 使用。
```
...
| [`voidtech-loop`](plugins/voidtech-loop) | 0.1.0 | ✅ 启用 | Goal Spec + 确定性控制器 + 隔离 worktree + 指定 commit 验收（开发中，F3 阶段） |
...
循环在独立 worktree 和专属分支上运行。`EVALS_PASSED` 只表示指定 commit 通过约定的 eval，仍需人工复核；循环不会自动 push、merge、创建 PR/MR 或改写用户分支。当前 F3 阶段仅支持 macOS arm64，要求 Claude Code 2.1.210 或更高版本，并依赖 Node.js 18+、Git 与 `jq`。详细用法见 [USAGE.md](docs/USAGE.md#9-voidtech-loop-工程内循环)。
