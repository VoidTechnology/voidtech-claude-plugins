...
`voidtech-core v0.20.0` · `voidtech-product v0.6.0` · `voidtech-design/engineering v0.2.0` · `voidtech-loop v0.3.0` · Claude Code ≥ 2.1.154（loop ≥ 2.1.210，review ≥ 2.1.211） · OMP ≥ 17.1.5 · Apache-2.0
...
`to-prd` 和 `prd-from-requirements` 处理的是两类输入，不应串着使用。`ui-prototype` 与 `logic-spike` 都只回答一个明确问题，产物不能直接当作生产实现。完整边界见 [USAGE.md](docs/USAGE.md)。
...
四个工作流插件合计提供 27 个自包含中文技能 + 2 个专业 subagent；Core 另提供中文协作约定与共享 Archify Runtime。
...
| [`voidtech-product`](plugins/voidtech-product) | 0.6.0 | ✅ 启用 | 模块化 PRD、需求同步、Logic Atlas 阅读与产品验收 + Product Manager subagent |
...
- **Design（2）** — `to-design-brief`、`ui-prototype`
...
plugins/
  ...
  voidtech-design/                设计 brief + UI 原型
