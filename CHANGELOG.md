...
## voidtech-product 0.8.1 - 2026-08-04

Logic Atlas 编译器定位机器可解析表时，遇到章节标题后的任何子标题就判定「本章无表」，而验收硬门扫的是整个章节体。同一份 PRD 因此可以「机械自检 0 错误、Atlas 0 缺口」，而整章的表一行都没进过逻辑模型——且这种丢弃不产生任何缺口。在一个 24 模块的真实工作树上实测命中 12 处，其中一处的核心流程 8 张表（28 行）完全没有进入模型。本版修掉这条静默路径以及另外两处相关缺陷。

### Fixed

- 章节表定位改为在章节体内跨子标题查找，并把表头相同的多张表按文档顺序合并：按流程分组写「子标题 + 一张表」、或用空行分组（分组块不重复表头）都能被完整编译，不再要求作者手工并表。章节边界止于同级或更高标题，以及归属其它标记的标题，`§7.0 数据读写` 不会吞掉 `§7.0.1 页面数据读写`（标记按最长匹配归属，消除两者的子串误判）。
- 章节存在但没有任何机器可解析表时，全部 12 个标记一律产出缺口；只有正文显式写「不涉及：{原因}」才豁免，判定与机械自检一致。章节内表头不同、未被消费的表也各留一条 `unparsed` 缺口——静默丢弃比表头违规更危险，后者至少留得下信号。
- 领域规格 `§N` 引用切片支持节号后的句点：模板自身写的是 `## 2. 状态机`，旧正则要求节号后紧跟空白，导致照模板写必然切片失败、状态机主本永远读不到。同时把 `第 N 节` 纳入识别，两种写法精度一致。引用与主本标题号对不上时按缺口处理，不再退回全文猜测。
- 「状态机与状态流转」章节按表头分派：引用领域规格的 3 列表与本模块独有对象的 9 列表可以并存，两者都编译。此前只取首表，同时持有两类的模块只能二选一，被牺牲的总是本地对象。

### Compatibility

- `GENERATOR_VERSION` 升至 1.9.0 并重签渲染器验证证明；渲染器、fixture、schema 与产物结构未变。
- 编译结果只增不减：此前被静默丢弃的表开始进入模型，同时暴露它们自带的引用与一致性缺口，缺口总数会上升——上升的部分是原本就存在、只是看不见的。
- 新鲜度判定仍只比对三摘要，不含 `generatorVersion`：已发布 Atlas 的工作树升级后需显式重新发布（`prd-sync.py atlas --publish`）才能拿到新模型。
- `templates/module-prd.md` 补充机器可解析章节的通用约定（多表分组、不涉及句式、`§N` 引用一致性），既有文档无需改写即可继续编译。
