...
以图表为主，只保留理解建议所需的简短说明。架构术语来自 `voidtech-engineering:codebase-design` 技能，并保持用词一致。
...
报告正文使用简单、简洁的英语。架构名词和动词直接采用 `voidtech-engineering:codebase-design` 技能中的定义，不要为追求简短而改变术语含义。
...
直接陈述问题和建议，不要加入“值得注意的是”等铺垫。如果一句话可以写成列表项，就保持简短；没有信息价值的列表项直接删除。优先使用 `voidtech-engineering:codebase-design` 已定义的术语，不要随意新增近义词。
