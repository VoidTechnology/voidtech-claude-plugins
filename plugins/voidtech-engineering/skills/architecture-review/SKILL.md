...
- 运行 `voidtech-engineering:codebase-design` 技能，了解架构词汇（**module**、**interface**、**depth**、**seam**、**adapter**、**leverage**、**locality**）及其原则（移除模块检验、“接口就是测试面”、“一个 adapter 通常不足以证明需要 seam，两个 adapter 才能证明替换需求真实存在”）。每条建议都使用这些术语，不要换成含义不完全相同的 "component"、"service"、"API" 或 "boundary"。
...
副作用在决策成形时就地发生——运行 `voidtech-engineering:feature-context` 技能，让功能上下文随进展保持最新：
...
- **想为深化后的模块比较多种接口？** 运行 `voidtech-engineering:codebase-design`，使用其中的多方案接口设计流程。
