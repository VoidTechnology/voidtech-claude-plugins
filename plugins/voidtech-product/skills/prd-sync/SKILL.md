...
5. 提交成功后，把已确认变更集（proposal 输出 + confirm 结果）交给 `voidtech-product:prd-maintain` 工况 2 合入主本并执行统一收尾。本技能到此为止。
...
- 不从原始需求首建工作树——首建走 `voidtech-product:prd-from-requirements`。
