...
- `proof_inherits(previous_proof, current_env)`：渲染器验证证明八键继承纯函数。
...
from .core_archify import architecture_ir, archify_bridge, lifecycle_ir
...
_PROOF_INHERIT_KEYS = ("rendererVersion", "generatorVersion", "schemaVersion",
                       "assetDigest", "archifyDigest", "fixtureDigest",
                       "validationHarnessVersion", "browserMatrixVersion")
...
def _section_text(text, marker):
    """切出标题含 marker 的章节文本(到下一同级或更高标题止)。"""
    lines = text.splitlines()
    start = None
    level = None
    for index, line in enumerate(lines):
        stripped = line.lstrip()
        if stripped.startswith("#") and marker in line:
            start = index
...
def _tables_for_state_reference(text, reference):
    ...
    return _all_markdown_tables(_section_text_for_reference(text, reference))


_MERMAID_FENCE_RE = re.compile(r"```mermaid\b[^\n]*\n(.*?)```", re.DOTALL)


def _parse_state_diagram(body):
    """把单个 stateDiagram-v2 块解析为 (edges, unparsed)。

    edges: [{"from","to","label"}]，`[*]` 原样保留为起点/终态标记；label 内
    `[推断]`、`(MBR-xxx)` 等标注原样保留。非状态图返回 ([], [])；无法解析为
    合法边的行 fail closed 进 unparsed，绝不据此猜测建边。"""
    edges = []
    unparsed = []
    started = False
    for raw in body.splitlines():
        ...
        if not started:
            if line.startswith("stateDiagram-v2"):
                ...
                continue
            return [], []
        if line.startswith("%%") or line.startswith("direction "):
            continue
        if "-->" not in line:
        ...
        src = before.strip()
        if ":" in after:
            dst, label = after.split(":", 1)
            dst, label = dst.strip(), label.strip()
        else:
            dst, label = after.strip(), ""
...
def _mermaid_state_diagrams(text):
    """抽取全部 stateDiagram-v2 mermaid 块并逐块解析；非状态图/空块跳过。"""
    diagrams = []
    for match in _MERMAID_FENCE_RE.finditer(text or ""):
        edges, unparsed = _parse_state_diagram(match.group(1))
        if not edges and not unparsed:
            continue
        diagrams.append({"edges": edges, "unparsed": unparsed})
    return diagrams


def _business_states_in_diagram(diagram):
    """图中除 `[*]` 外的业务状态名集合。"""
    names = set()
...
def _match_state_diagram(object_states, diagrams):
    """选与对象声明当前状态交集最大的图；无交集不匹配。"""
    best = None
    best_score = 0
    for index, diagram in enumerate(diagrams):
        overlap = len(_business_states_in_diagram(diagram) & object_states)
...
def _project_state_diagram(meta, diagram):
    """按对象的当前/下一状态裁切共享 mermaid 图，避免跨对象串边。"""
    current = meta["states"]
    allowed = current | meta["targets"]
    edges = []
    for edge in diagram["edges"]:
        source, target = edge["from"], edge["to"]
        if source == "[*]":
            include = target in current
        elif target == "[*]":
            include = source in current
        else:
            include = source in current and target in allowed
        if include:
            edges.append(edge)
    return {"edges": edges, "unparsed": diagram["unparsed"] if edges else []}
...
def _compile_module(root, module_scope, text, module_scopes, page_catalog,
    ...
    def _add_state_nodes(fields, sources):
        # 按状态表只建状态节点与元数据(进入条件/可逆/通知等)；绝不据此建流转边。
        object_name = fields["object"]
        current = fields["current"]
        _ensure_business_state(
        ...
        meta = state_object_meta.setdefault(
            object_name, {
                "states": set(), "targets": set(),
                "actions": 0, "nexts": 0})
        meta["states"].add(current)
        meta["actions"] += len(_declared_items(fields.get("actions", "")))
        targets = [
            target for target in _next_states(fields.get("nextState", ""))
            if _terminal_result(target) is None]
        meta["targets"].update(targets)
        meta["nexts"] += len(targets)

    def _emit_diagram_transitions(object_name, diagram, sources):
        # 逐边流转只来自 mermaid stateDiagram；`[*]` 记为起点/终态标记(非业务状态)。
        def _state_node(name, has_outgoing=False):
            node_id = f"state:{module_scope}:{object_name}:{name}"
            if node_id not in business_state_nodes:
                node = {
                    "nodeId": node_id, "kind": "state",
                    "scopeId": module_scope, "title": name,
                    "status": "original", "sources": [dict(sources[0])],
                    "detail": {"category": "businessState",
                               "object": object_name,
                               "declaredAsTargetOnly": True},
                }
                business_state_nodes[node_id] = node
                nodes.append(node)
            if has_outgoing:
                business_state_nodes[node_id]["detail"].pop(
                    "declaredAsTargetOnly", None)
            return node_id

        for edge in diagram["edges"]:
            src, dst, label = edge["from"], edge["to"], edge["label"]
            if src == "[*]" and dst == "[*]":
                continue
            if src == "[*]":
                detail = business_state_nodes[_state_node(dst)]["detail"]
                detail["declaredStart"] = True
                triggers = detail.setdefault("startTriggers", [])
                if label and label not in triggers:
                    triggers.append(label)
                continue
            if dst == "[*]":
                detail = business_state_nodes[_state_node(
                    src, has_outgoing=True)]["detail"]
                detail["declaredTerminal"] = True
                if label:
                    results = detail.setdefault("terminalResults", [])
                    if label not in results:
                        results.append(label)
                    detail.setdefault("terminalResult", label)
                continue
            from_id = _state_node(src, has_outgoing=True)
            to_id = _state_node(dst)
            edge_sources = [
                dict(sources[0], requirementIds=_requirement_ids(label))]
            edge_sources += [dict(extra) for extra in sources[1:]]
            _add_edge({
                "edgeId": (f"transition:{module_scope}:{object_name}:"
                           f"{src}->{dst}:{label}"),
                "kind": "transition", "from": from_id, "to": to_id,
                "status": "original", "sources": edge_sources,
                # detail.action 供既有 viewer/lifecycle 渲染逐边标签；
                # detail.trigger 为语义字段(mermaid 边标签、单一触发)。
                "detail": {"trigger": label, "action": label},
            })
        if diagram["unparsed"]:
            gaps.append({
                "gapId": (f"gap:{module_scope}:state-diagram-unparsed:"
                          f"{object_name}"),
                "scopeId": module_scope, "kind": "unparsed",
                "detail": (
                    f"对象「{object_name}」状态图存在无法解析的行"
                    "(已跳过、未据此建边): "
                    + " / ".join(diagram["unparsed"][:5])),
                "backlogRef": None})

    def _diagram_gap(object_name, gap_id):
        # 无 mermaid 逐边来源：只按状态表则无法确定逐项映射——进缺口，绝不编造。
        meta = state_object_meta.get(
            object_name, {"actions": 0, "nexts": 0})
        n_actions, m_nexts = meta["actions"], meta["nexts"]
        if n_actions > 1 or m_nexts > 1:
            kind = "ambiguous-relation"
            detail = (
                f"对象「{object_name}」以按状态表声明 {n_actions} 个可执行操作、"
                f"{m_nexts} 个下一状态迁移，但缺 mermaid stateDiagram-v2 逐边来源，"
                "无法确定「操作 × 下一状态」的逐项映射(避免笛卡尔积编造)；"
                "已保留状态节点，逐边流转待补 mermaid。")
        else:
            kind = "missing-transition"
            detail = (
                f"对象「{object_name}」缺 mermaid stateDiagram-v2 逐边来源，"
                "无法确定性抽取逐边状态流转；已保留状态节点，逐边流转待补 mermaid。")
    ...
    if state_status == "absent":
        ...
        for object_name in sorted(state_object_meta):
            ...
            if index is None:
                _diagram_gap(
                    object_name,
                    f"gap:{module_scope}:state-transitions:{object_name}")
                continue
            diagram = _project_state_diagram(meta, diagrams[index])
        ...
        for reference_index, row in enumerate(state_rows, 1):
            ...
            for domain_row in matched_rows:
                object_name = row[0] if alias_fallback else domain_row[0]
                if object_name not in row_objects:
                    row_objects.append(object_name)
                domain_source = {
            ...
            if not matched_rows:
                gaps.append({
                    ...
                    "backlogRef": None})
                continue
            diagrams = _mermaid_state_diagrams(section_text)
            for object_name in row_objects:
                meta = state_object_meta.get(object_name)
                states = meta["states"] if meta else set()
                index = _match_state_diagram(states, diagrams)
                if index is None:
                    _diagram_gap(
                        object_name,
                        f"gap:{module_scope}:state-transitions:"
                        f"{reference_index}:{object_name}")
                    continue
                edge_domain_source = {
                    "path": relpath, "anchor": row[1],
                    "requirementIds": [], "oqIds": []}
    ...
    for state_node in business_state_nodes.values():
        ...
        if not detail_obj.get("declaredAsTargetOnly"):
            continue
        if detail_obj.get("declaredTerminal"):
            continue
...
def _render_report(model, atlas_manifest, presentation):
    cov = model["coverage"]
    risks = presentation.get("presentationRisk") or []
    ...
    out += ["",
            ...
            ""]
    if not risks:
        out.append("- 全部状态机已通过 Archify fail-closed 渲染。")
    else:
        for risk in risks:
            diagnostics = risk.get("diagnostics") or []
            codes = "、".join(item.get("code", "unknown") for item in diagnostics)
            label = risk.get("object") or "全局运行时"
            scope = risk.get("scopeId") or "全部模块"
            out.append(
                f"- [{scope}] {label}: 已降级为 Atlas 内建状态图（{codes}）")
    out.append("")
    return "\n".join(out) + "\n"
...
_LIFECYCLE_TOKEN = "__ATLAS_LIFECYCLE_JSON__"
...
def _render_html(model, atlas_manifest, presentation):
    """把模型、快照元数据与 Lifecycle 产物注入自包含 viewer 模板。"""
    risks = presentation.get("presentationRisk") or []
    meta = {
        ...
        "presentationRisk": len(risks),
        "lifecycleMachineCount": len(presentation.get("machines") or []),
    }
    html = _viewer_template_bytes().decode("utf-8")
    html = _inject_json(html, _MODEL_TOKEN, model)
    html = _inject_json(html, _META_TOKEN, meta)
    html = _inject_json(html, _LIFECYCLE_TOKEN, presentation)
    return html
...
def _plan_from_model(root, stage, model):
    ...
    presentation = archify_bridge.build_presentation(model)

    contents = {
        LEDGER_RELPATH: ledger_content,
        MODEL_RELPATH: canonical_json_bytes(model),
        PRESENTATION_RELPATH: canonical_json_bytes(presentation),
    }
    ...
    views = {
        MD_RELPATH: _render_markdown(model, manifest).encode("utf-8"),
        REPORT_RELPATH: _render_report(
            model, manifest, presentation).encode("utf-8"),
    }
    if stage in ("html", "polish"):
        views[HTML_RELPATH] = _render_html(
            model, manifest, presentation).encode("utf-8")
    ...
    plan = [{"path": rel, "action": "write", "content": contents[rel]}
            for rel in (LEDGER_RELPATH, MODEL_RELPATH, PRESENTATION_RELPATH,
                        MD_RELPATH, REPORT_RELPATH)]
...
def build_plan(root):
    """按 stage 产出 engine plan：模型、Lifecycle 呈现、Markdown、报告、
    manifest 与 ledger；html/polish 另加自包含 HTML。"""
    root = Path(root)
...
def _fixture_architecture(model):
    """浏览器 harness 使用的固定 Architecture 总览产物，不依赖本机 Node。

    形状与 archify_bridge.build_architecture 的 ok 分支一致，SVG 保留真实的
    data-node-id / data-edge-* 钩子供 viewer 点击下钻与断言。"""
    def _esc(text):
    ...
    for comp in ir["components"]:
        ...
        w, h = comp["size"]
        parts.append(
            f'<g id="node-{_esc(comp["id"])}" data-node-id="{_esc(comp["id"])}" '
            f'data-node-label="{_esc(comp["label"])}" role="button" tabindex="0">'
            f'<rect class="c-mask" x="{x}" y="{y}" width="{w}" height="{h}"/>'
            f'<rect class="c-{comp["type"]}" x="{x}" y="{y}" '
            f'width="{w}" height="{h}"/>'
            f'<text class="t-primary" x="{x + w / 2}" y="{y + h / 2}" '
            f'text-anchor="middle">{_esc(comp["label"])}</text></g>')
    for conn in ir["connections"]:
        parts.append(
            f'<g data-edge-id="{_esc(conn["id"])}" '
            f'data-edge-from="{_esc(conn["from"])}" '
            f'data-edge-to="{_esc(conn["to"])}" '
            f'data-edge-label="{_esc(conn["label"])}">'
            '<path class="a-default" d="M0,0 L10,10"/>'
            f'<text class="t-muted">{_esc(conn["label"])}</text></g>')
    view_box = ir["meta"]["viewBox"]
    svg = (
        '<svg xmlns="http://www.w3.org/2000/svg" '
        'data-archify-diagram="architecture" data-preset="blueprint" '
        f'viewBox="0 0 {view_box[0]} {view_box[1]}" '
        'role="img" aria-label="Architecture fixture">'
        + "".join(parts) + "</svg>")
    tokens, presets = archify_bridge.svg_style_inputs([svg])
    css = archify_bridge.lifecycle_svg_css(
        tokens, presets, scope_class="archify-architecture-svg")
    if not css:
        raise AtlasValidationError(
            ["fixture 无法从 vendored Archify 模板抽取 architecture SVG 样式"])
    return {
        "schemaVersion": 1,
        "renderer": "archify-architecture",
        "runtime": {"status": "fixture", "major": 18},
        "css": css,
        "cssDigest": sha256_of_bytes(css.encode("utf-8")),
        "status": "ok",
        "attempts": 1,
        "ir": ir,
        "irDigest": sha256_of_bytes(architecture_ir.canonical_ir_bytes(ir)),
        "svg": svg,
        "svgDigest": sha256_of_bytes(svg.encode("utf-8")),
        "diagnostics": [],
...
def _fixture_presentation(model):
    """浏览器 harness 使用的固定 Lifecycle 产物，不依赖本机 Node。"""
    machines = lifecycle_ir.extract_machines(model)
    rendered = []
    for machine in machines:
        ir = lifecycle_ir.build_lifecycle_ir(machine)
        labels = sorted(item["label"] for item in ir["states"])
        text = "".join(
            f'<text x="40" y="{50 + index * 34}" class="t-primary" '
            'data-lifecycle-state="true">'
            f'{label.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")}'
            "</text>"
            for index, label in enumerate(labels))
        svg = (
            '<svg xmlns="http://www.w3.org/2000/svg" '
            'data-archify-diagram="lifecycle" data-preset="blueprint" '
            'viewBox="0 0 640 240" '
            'role="img" aria-label="Lifecycle fixture">'
            f"<g>{text}</g></svg>")
        rendered.append({
            ...
            "attempts": 1,
            "ir": ir,
            "irDigest": sha256_of_bytes(lifecycle_ir.canonical_ir_bytes(ir)),
            "svg": svg,
            "svgDigest": sha256_of_bytes(svg.encode("utf-8")),
            "diagnostics": [],
            "stateNodeIds": {
    ...
    css = archify_bridge.lifecycle_svg_css({"t-primary"}, {"blueprint"})
    if not css:
        raise AtlasValidationError(
            ["fixture 无法从 vendored Archify 模板抽取 SVG 样式"])
    return {
        "schemaVersion": 1,
        ...
        "presentationRisk": [],
        "machines": rendered,
        "architecture": _fixture_architecture(model),
    }
...
def render_fixture_html():
    ...
    if errors:
        raise AtlasValidationError(errors)
    return _render_html(model, _fixture_manifest(), _fixture_presentation(model))


def render_fallback_fixture_html():
    """产出 Node/Archify 不可用时的逐状态机降级浏览器 fixture。"""
    model = _fixture_model()
    presentation = _fixture_presentation(model)
    risks = []
    for machine in presentation["machines"]:
        machine["status"] = "degraded"
        machine.pop("svg", None)
        machine.pop("svgDigest", None)
        machine["diagnostics"] = [{
            "code": "runtime/node-unavailable",
    ...
    architecture.pop("svg", None)
    architecture.pop("svgDigest", None)
    architecture["css"] = ""
    architecture["cssDigest"] = None
    architecture["diagnostics"] = [{
        "code": "runtime/node-unavailable",
        "message": "Node.js 18+ 不可用，已降级为 Atlas 内建总览图",
    }]
    architecture["runtime"] = {
        "status": "unavailable", "code": "runtime/node-unavailable"}
    risks.append({
        "machineId": None,
        "scopeId": None,
        "object": "系统关系总览",
    ...
    presentation["runtime"] = {
        "status": "unavailable", "code": "runtime/node-unavailable"}
    # 与 build_presentation 的形状对齐：没有任何 ok 状态机时不携带样式。
    presentation["css"] = ""
    presentation["cssDigest"] = None
    presentation["presentationRisk"] = risks
    return _render_html(model, _fixture_manifest(), presentation)


def renderer_env():
    """当前渲染器验证环境：八个继承键（ADR-0005 §8）。

    assetDigest 覆盖 viewer、HTML 注入函数及 Lifecycle 布局/桥源码；
    archifyDigest 独立覆盖 vendored Archify，便于准确归因升级影响。
    fixtureDigest 覆盖 model、manifest 与注入的 Lifecycle fixture。
    """
    asset_source = inspect.getsource(_render_html) + inspect.getsource(_snapshot_lines)
    source_bytes = (
        Path(lifecycle_ir.__file__).read_bytes()
        + Path(architecture_ir.__file__).read_bytes()
        + Path(archify_bridge.__file__).read_bytes())
    asset_bytes = (
        _viewer_template_bytes() + asset_source.encode("utf-8") + source_bytes)
    model = _fixture_model()
    lifecycle = _fixture_presentation(model)
    fixture = {
        "manifest": _fixture_manifest(),
        "model": model,
        "lifecycle": lifecycle,
        "fallbackHtmlDigest": sha256_of_bytes(
            render_fallback_fixture_html().encode("utf-8")),
    }
    return {
        ...
        "archifyDigest": archify_bridge.vendor_digest(),
