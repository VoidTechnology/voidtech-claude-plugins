...
def check_declared_id_format(files, md_files):
    ...
    已知盲区(自校准的代价,不是缺陷): 一条正则只管「已经有合规实例」的
    前缀,因此某前缀下全部字面量都是旧形态时,规则不认领该前缀、什么都
    不报。本检查覆盖「新旧共存」,不覆盖「全量陈旧」——后者仍要靠定案时
    的回扫。放弃这条覆盖是为了换掉朴素版本在真实树上的 1314 条假阳性;
    两侧行为都由 tests/test_check_prd_tree.py::DeclaredIdFormatTest 固化。
    """
    errors = []
