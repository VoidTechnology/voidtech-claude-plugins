...
4. 交独立核验（规则单源见 `prd-from-requirements`「深度分级与分期交付」）：委派 `@voidtech-product:product-manager` 按 DoD 与一致性自检逐项核验留证，记录写入 backlog「验收级核验记录」表。通过后才更新模块头部深度、第 13 节行级追溯、追溯矩阵区间入口标注和 backlog「已完成」；打回则修订后重新核验。做的人不能给自己认证。
...
- 源文件出了新版本（如 Excel 更新）：一律交 `voidtech-product:prd-sync`（sync → propose → confirm），由它计算差异并经用户确认；确认后的变更集回到本工况，按下面第 3、4 步做影响面确认与主本合入。
