...
1. **机构记忆** —— 某个功能为何被拒绝，这样在 issue 关闭后理由也不会丢失
2. **去重** —— 当新来的 issue 匹配上某个既往拒绝时，技能可以浮现先前的决策，而不是重新争论一遍
...
文件应当以一种轻松、易读的风格来写——更像一份简短的设计文档，而不是一条数据库记录。用段落、代码示例和例子把理由讲清楚，让第一次接触它的人也觉得有用。
...
理由应当经久耐用。避免引用临时性的处境（“我们现在太忙了”）——那些不是真正的拒绝，而是延期。

## 何时检查 `.out-of-scope/`

在 triage 期间（步骤 1：收集上下文），阅读 `.out-of-scope/` 中的所有文件。在评估一个新 issue 时：

- 检查该请求是否匹配某个既有的 out-of-scope 概念
- 匹配按概念相似度，而非关键字——"night theme" 匹配 `dark-mode.md`
- 若有匹配，把它浮现给维护者："This is similar to `.out-of-scope/dark-mode.md` — we rejected this before because [reason]. Do you still feel the same way?"

维护者可以：

- **确认** —— 新 issue 被追加到既有文件的 "Prior requests" 列表，然后关闭
- **重新考虑** —— out-of-scope 文件被删除或更新，issue 进入正常 triage 流程
- **不同意** —— 两个 issue 相关但有别，按正常 triage 进行

## 何时写入 `.out-of-scope/`

仅当一个 **enhancement**（不是 bug）被*拒绝*为 `wontfix` 时。这条规则对 enhancement PR 与对 issue 完全一样适用——一个被拒绝的 PR 会被记录在此，以免同一请求又作为新代码回来。

当某物因为**已实现**而被关闭为 `wontfix` 时，**不要**写入这里。那是一个已构建的功能，不是被拒绝的；记录它会用虚假的拒绝污染去重检查。相反，关闭评论应指向该功能已经存在之处。
...
- 触发这次重新考虑的新 issue 进入正常 triage 流程
