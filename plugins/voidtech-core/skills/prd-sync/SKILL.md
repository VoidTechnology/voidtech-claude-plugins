...
报告能力开关、各源游标（applied/pending/observed）、open proposal、非终态 operation、读取栅栏与 Atlas 新鲜度（能力开启时）。只读；有栅栏时如实报告并建议工况 5。Atlas 的检查与发布用 `atlas <worktree> [--check|--publish|--gate]`；Logic Atlas 能力阶段的置位用 `atlas <worktree> --enable <markdown|html|polish>`（前提：模块 prd.md 已按 `prd-from-requirements` 模板补齐三张「机器可解析」表，置位后运行 `atlas --publish` 生成/更新生成物）。
