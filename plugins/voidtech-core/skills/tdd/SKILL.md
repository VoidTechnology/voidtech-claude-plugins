---
name: tdd
description: 使用测试驱动开发构建功能或修复缺陷，按红灯、绿灯、重构的小步循环推进，并优先验证公开行为。
---
...
**正确做法**：按端到端探路切片推进。一个测试 → 一份实现 → 重复。每个测试都基于上一轮获得的信息；代码刚刚写完，你更容易判断哪些行为重要、应该如何验证。
...
- [ ] 与用户确认需要哪些接口变更
- [ ] 与用户确认要测试哪些行为（排好优先级）
- [ ] 识别深模块的机会（小接口、深实现）——运行 `voidtech-core:codebase-design` 技能获取相关词汇与可测性检查
- [ ] 列出要测试的行为（不是实现步骤）
- [ ] 获得用户对计划的批准
