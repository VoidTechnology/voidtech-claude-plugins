...
description: 建立并维护项目的领域模型。当用户需要统一领域术语、记录架构决策，或其他技能需要更新领域模型时使用。
---
...
在设计过程中同步维护项目的领域模型：检查术语是否准确，用边界场景验证概念，并把确认后的术语和决策及时写入词汇表或 ADR。仅仅读取 `CONTEXT.md` 不需要调用本技能；只有新增、修改或澄清领域模型时才使用。
...
按需创建文件。确认第一个领域术语时再创建 `CONTEXT.md`；需要记录第一份 ADR 时再创建 `docs/adr/`。
...
讨论领域关系时，使用具体场景验证边界情况，帮助用户明确相近概念之间的区别。
