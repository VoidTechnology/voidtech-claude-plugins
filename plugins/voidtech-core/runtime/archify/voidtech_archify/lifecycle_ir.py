...
def _tight_viewbox(ir_states, ir_transitions):
    ...
    # vendor 水平界限校验要求 state.x+width ≤ viewBox[0]-32（render-lifecycle
    # validateLayout），24px 边距差 8px 触发降级（2026-07-24 支付段三台）；取 48
    # 留足余量。
    reserve = _RIGHT_CHANNEL_RESERVE if uses_right_channel else 48
